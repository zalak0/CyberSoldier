...
def predict_access_vector(knn_model, df, input_sample, target, label_encoder=None):
    """
    ...
    print(f"A vulnerability of {cwe_name} (CWE-{input_sample[0]}) with a CVSS score of {input_sample[1]}, access complexity {input_sample[2]}, \
authentication {input_sample[3]}, product ID {input_sample[4]}, from vendor ID {input_sample[5]}, will result in a {prediction[0]} risk to {target_name}")
...
for target in targets:
    ...
    forest, df = forest_classifier("cleaned_cve.csv", target)
    #ensemble, df = ens_classifier("cleaned_cve.csv")
    predict_access_vector(forest, df, [79, 5, 1, 1, 1, 4], target)
