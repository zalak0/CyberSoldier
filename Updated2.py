...
def file_initialise(csv_file, target): 
    df = pd.read_csv(csv_file)
    df = df.rename(columns={"Unnamed: 0": "cve_id"})
    df_cleaned = df.dropna()
    
    df_products = pd.read_csv('cleaned_products.csv')
    df_vendor_only = pd.read_csv('cleaned_vendors.csv').rename(columns={"Unnamed: 0": "cve_id"})
...
def predict_access_vector(model, df, input_sample, target, le_comp, le_auth, le_prod, le_vend):
    """
    ...
    print(
        f"The vulnerability '{cwe_name}' (CWE-{input_sample[0]}), with a CVSS score of {input_sample[1]}, "
        f"requires access complexity '{access_comp}' and authentication '{access_auth}'. "
        f"It affects product '{product}' from vendor '{vendor}', and is predicted to pose a '{prediction}' risk to {target_name}.")

def knn_classifier(csv_file, target):
    X, Y, df = file_initialise(csv_file, target)
...
def mlp_classifier(csv_file, target):
    X, Y, df = file_initialise(csv_file, target)
...
def xg_classifier(csv_file, target):
    X, Y, df = file_initialise(csv_file, target)
...
def ens_classifier(csv_file, target):
    X, Y, df = file_initialise(csv_file, target)
...
for target in targets:
    #X, Y, df_merged, le_comp, le_auth, le_prod, le_vend = file_initialise("cleaned_cve.csv", target)
    print(f"\n Classifiers for: {target.upper()}")
    #knn, df = knn_classifier("cleaned_cve.csv", target)
    #mlp, df = mlp_classifier("cleaned_cve.csv", target)
    #xgboost, df = xg_classifier("cleaned_cve.csv", target)
    forest, df_merged, le_comp, le_auth, le_prod, le_vend = forest_classifier("cleaned_cve.csv", target)
    #ensemble, df = ens_classifier("cleaned_cve.csv")
    input_sample = [79, 5, 1, 1, 1, 4]
    predict_access_vector(forest, df_merged, input_sample, target, le_comp, le_auth, le_prod, le_vend)
